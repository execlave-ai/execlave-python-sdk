...
# =========================================================================
# Error-trace completeness (QA 2026-09-23 #1)
# =========================================================================


class TestErrorTraceDefaults:
    """finish("error", msg) omitted the type; older platforms rejected that
    trace and the whole batch with it. Error traces now always carry both."""

    def test_documented_call_gets_placeholder_type(self):
        exe = _make_exe_stub()
        Trace(exe).finish("error", "upstream timed out")
        payload = exe._buffer_trace.call_args[0][0]
        assert payload["status"] == "error"
        assert payload["errorType"] == "UnspecifiedError"
        assert payload["errorMessage"] == "upstream timed out"

    def test_blank_values_get_both_placeholders(self):
        exe = _make_exe_stub()
        Trace(exe).finish("error", "  ", "")
        payload = exe._buffer_trace.call_args[0][0]
        assert payload["errorType"] == "UnspecifiedError"
        assert payload["errorMessage"] == "No error message provided"

    def test_explicit_values_are_kept(self):
        exe = _make_exe_stub()
        Trace(exe).finish("error", "boom", "ValueError")
        payload = exe._buffer_trace.call_args[0][0]
        assert payload["errorType"] == "ValueError"
        assert payload["errorMessage"] == "boom"

    def test_success_trace_has_no_error_fields(self):
        exe = _make_exe_stub()
        Trace(exe).finish("success")
        payload = exe._buffer_trace.call_args[0][0]
