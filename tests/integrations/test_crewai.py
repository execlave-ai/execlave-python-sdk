...
class TestInstrumentCrew:
    ...
    def test_tool_step_seals_full_tool_input_into_metadata(self, ag_client, monkeypatch):
        # Previously this adapter called exe.enforce_policy directly with no
        # metadata at all -- a require_approval certificate for a CrewAI
        # tool call bound to nothing but a str()'d input, not the full tool
        # input structure. Prove it's sealed now, via enforce_policy_bound.
        crew = _FakeCrew()
        captured: dict = {}
        ...
        monkeypatch.setattr(ag_client, "enforce_policy", fake_enforce)
        instrument_crew(crew, ag_client, agent_id="bot", enforce=True)
        step = MagicMock(tool="search", tool_input={"query": "sales figures"}, output="r")
        crew.step_callback(step)
        assert captured["metadata"] == {"toolInput": {"query": "sales figures"}}
