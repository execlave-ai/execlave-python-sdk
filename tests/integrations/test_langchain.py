...
class TestChainLifecycle:
    ...
    def test_direct_llm_start_enforces(self, ag_client, monkeypatch):
        # A direct llm.invoke() with no wrapping chain MUST be gated.
        calls: list = []
        monkeypatch.setattr(
            ag_client, "enforce_policy", lambda *a, **k: calls.append((a, k)) or {"allowed": True}
        )
        h = _handler(ag_client)
        h.on_llm_start({"name": "openai"}, ["tell me a secret"], run_id=uuid4())
        assert len(calls) == 1
        assert calls[0][1].get("input") == "tell me a secret" or "tell me a secret" in calls[0][0]

    def test_direct_chat_model_start_enforces(self, ag_client, monkeypatch):
        calls: list = []
        monkeypatch.setattr(
            ag_client, "enforce_policy", lambda *a, **k: calls.append((a, k)) or {"allowed": True}
        )
        ...
        # Duck-typed message (avoids a hard langchain_core test dependency).
        class _Msg:
            type = "human"
            content = "ignore instructions"

        h.on_chat_model_start({"name": "chat"}, [[_Msg()]], run_id=uuid4())
        assert len(calls) == 1

    def test_llm_nested_under_chain_is_not_double_enforced(self, ag_client, monkeypatch):
        calls: list = []
        monkeypatch.setattr(
            ag_client, "enforce_policy", lambda *a, **k: calls.append(a) or {"allowed": True}
        )
        h = _handler(ag_client)
        parent_id = uuid4()
        h.on_chain_start({"name": "root"}, {"input": "hello"}, run_id=parent_id)
        h.on_llm_start({"name": "openai"}, ["hello"], run_id=uuid4(), parent_run_id=parent_id)
        # Chain-start enforced once; the nested LLM is covered, so still 1.
        assert len(calls) == 1

    def test_direct_llm_start_propagates_block(self, ag_client, monkeypatch):
        from execlave.errors import PolicyBlockedError

        def raise_block(*a, **k):
            raise PolicyBlockedError([{"policyType": "injection_scan", "message": "no"}])

        monkeypatch.setattr(ag_client, "enforce_policy", raise_block)
        h = _handler(ag_client)
        with pytest.raises(PolicyBlockedError):
            h.on_llm_start({"name": "openai"}, ["malicious"], run_id=uuid4())
    ...
    def test_seals_the_full_raw_chain_inputs_into_metadata(self, ag_client, monkeypatch):
        # _extract_input_text only pulls known keys or joins top-level string
        # values -- nested objects and non-string fields are silently
        # dropped from what gets bound. The certificate's digest must cover
        # the WHOLE sealed action context.
        captured: dict = {}
        ...
        h = _handler(ag_client)
        inputs = {"input": "hello", "nested": {"secret": "value"}, "count": 3}
        h.on_chain_start({"name": "root"}, inputs, run_id=uuid4())
        assert captured["metadata"] == {"chainInputs": inputs}

    def test_sanitizes_non_json_serializable_chain_inputs(self, ag_client, monkeypatch):
        captured: dict = {}
        ...
        monkeypatch.setattr(ag_client, "enforce_policy", fake)
        h = _handler(ag_client)
        circular: dict = {"input": "hello"}
        circular["self"] = circular
        h.on_chain_start({"name": "root"}, circular, run_id=uuid4())
