...
class TestExeclaveLlamaIndexHandler:
    ...
    def test_seals_full_untruncated_query_into_metadata(self, ag_client, monkeypatch):
        ...
        h = ExeclaveLlamaIndexHandler(ag_client, agent_id="bot")
        big_query = "x" * 5000
        h.handle(_event("QueryStartEvent", span_id="s1", query=big_query))
        assert captured["metadata"] == {"query": big_query}

    def test_sanitizes_non_json_serializable_query(self, ag_client, monkeypatch):
        # Regression: a raw structured QueryBundle (or similar non-JSON-
        # native object) reaching json.dumps at the HTTP layer raises a
        # TypeError that `requests` does NOT classify as a
        # RequestException, so it is swallowed by the adapter's generic
        # except Exception as "non-fatal" -- silently allowing the call.
        captured: dict = {}
        ...
        class _QueryBundle:
            def __repr__(self):
                return "QueryBundle(query_str='hi')"

        h.handle(_event("QueryStartEvent", span_id="s1", query=_QueryBundle()))
    ...
    def test_seals_full_untruncated_tool_arguments_into_metadata(self, ag_client, monkeypatch):
        ...
        big_args = {"q": "x" * 5000}
        h.handle(
            _event("AgentToolCallEvent", span_id="t1", tool_name="search", arguments=big_args)
        )
    ...
    def test_sanitizes_non_json_serializable_tool_arguments(self, ag_client, monkeypatch):
        ...
        h.handle(
            _event("AgentToolCallEvent", span_id="t1", tool_name="wire_funds", arguments=circular)
        )
        assert captured["metadata"] is not None
        json.dumps(captured["metadata"])
