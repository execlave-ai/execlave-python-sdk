...
class TestInit:
    ...
    def test_cloud_http_base_url_is_upgraded_to_https(self, mock_session):
        exe = Execlave(
            api_key="exe_key_0123456789012345",
            base_url="http://api.execlave.com/",
            async_mode=False,
            enable_control_channel=False,
        )
        assert exe.base_url == "https://api.execlave.com"
...
class TestRegisterAgent:
    ...
    def test_registration_accepts_list_envelope_response(self, exe_client, mock_session):
        """Some deployed API paths can return a list envelope; select the requested agent."""
        other_agent = {
            **SAMPLE_AGENT_DATA,
            "id": "agt_other",
            "agentId": "other-bot",
        ...
        mock_session.request.return_value = make_mock_response(
            200,
            {"data": [other_agent, SAMPLE_AGENT_DATA]},
        )
    ...
    def test_registration_rejects_list_without_requested_agent(self, exe_client, mock_session):
        other_agent = {
            **SAMPLE_AGENT_DATA,
            "id": "agt_other",
            "agentId": "other-bot",
        ...
        mock_session.request.return_value = make_mock_response(200, {"data": [other_agent]})

        with pytest.raises(ExeclaveError, match="did not include agentId 'my-bot'"):
    ...
    def test_autonomy_level_sent_when_provided(self, exe_client, mock_session):
        mock_session.request.return_value = make_mock_response(200, {"data": SAMPLE_AGENT_DATA})
        exe_client.register_agent(
            agent_id="my-bot",
            name="My Bot",
        ...
        _, kwargs = mock_session.request.call_args
        payload = kwargs.get("json", {})
        assert payload["autonomyLevel"] == "autonomous"

    def test_autonomy_level_omitted_by_default(self, exe_client, mock_session):
        mock_session.request.return_value = make_mock_response(200, {"data": SAMPLE_AGENT_DATA})
        exe_client.register_agent(agent_id="my-bot", name="My Bot")

        _, kwargs = mock_session.request.call_args
        payload = kwargs.get("json", {})
        assert "autonomyLevel" not in payload
...
class TestGetAgentCredential:
    ...
    def test_fetches_and_caches_credential(self, exe_client, mock_session):
        expires_at = "2099-01-01T00:00:00Z"
        mock_session.request.return_value = make_mock_response(
            200,
            {"data": {"credential": "exe_agt_token_1", "expiresAt": expires_at, "kid": "kid-1"}},
        )

        first = exe_client.get_agent_credential("33333333-3333-3333-3333-333333333333")
        second = exe_client.get_agent_credential("33333333-3333-3333-3333-333333333333")

        assert first == {"credential": "exe_agt_token_1", "expiresAt": expires_at, "kid": "kid-1"}
        assert second is first
        assert mock_session.request.call_count == 1
        args, kwargs = mock_session.request.call_args
        assert "/agents/33333333-3333-3333-3333-333333333333/credential" in (
            kwargs.get("url") or args[1]
        )

    def test_caches_separately_by_agent_id(self, exe_client, mock_session):
        mock_session.request.side_effect = [
            make_mock_response(
                200,
                {"data": {"credential": "exe_agt_a", "expiresAt": "2099-01-01T00:00:00Z"}},
            ),
            make_mock_response(
                200,
                {"data": {"credential": "exe_agt_b", "expiresAt": "2099-01-01T00:00:00Z"}},
            ),
        ]

        first = exe_client.get_agent_credential("agent-a")
        second = exe_client.get_agent_credential("agent-b")
        first_cached = exe_client.get_agent_credential("agent-a")

        assert first["credential"] == "exe_agt_a"
        assert second["credential"] == "exe_agt_b"
        assert first_cached is first
        assert mock_session.request.call_count == 2

    def test_refreshes_after_expiry(self, exe_client, mock_session):
        mock_session.request.side_effect = [
            make_mock_response(
                200,
                {"data": {"credential": "exe_agt_old", "expiresAt": "2000-01-01T00:00:00Z"}},
            ),
            make_mock_response(
                200,
                {"data": {"credential": "exe_agt_new", "expiresAt": "2099-01-01T00:00:00Z"}},
            ),
        ]

        first = exe_client.get_agent_credential("agent-uuid")
        second = exe_client.get_agent_credential("agent-uuid")

        assert first["credential"] == "exe_agt_old"
        assert second["credential"] == "exe_agt_new"
        assert mock_session.request.call_count == 2


# =========================================================================
# report_agent_metadata (Phase 2.1)
# =========================================================================


class TestReportAgentMetadata:

    def test_posts_to_versions_endpoint_with_cached_uuid(self, exe_client, mock_session):
        # Register first so the UUID (agt_abc123) is cached.
        mock_session.request.return_value = make_mock_response(200, {"data": SAMPLE_AGENT_DATA})
        exe_client.register_agent(agent_id="my-bot", name="My Bot")

        mock_session.request.return_value = make_mock_response(
            201, {"id": "ver_1", "versionNumber": 2}
        )
        exe_client.report_agent_metadata(
            agent_id="my-bot",
        ...
        args, kwargs = mock_session.request.call_args
        # URL path should target the version endpoint for the resolved UUID.
        assert "/agents/agt_abc123/versions" in (kwargs.get("url") or args[1])
        payload = kwargs.get("json", {})
        assert payload["versionLabel"] == "v2.1.0"
        assert payload["activate"] is True
        assert payload["metadata"]["gitCommit"] == "abc123"

    def test_defaults_activate_false_and_omits_empty_metadata(self, exe_client, mock_session):
        mock_session.request.return_value = make_mock_response(200, {"data": SAMPLE_AGENT_DATA})
        exe_client.register_agent(agent_id="my-bot", name="My Bot")

        mock_session.request.return_value = make_mock_response(201, {"id": "ver_1"})
        exe_client.report_agent_metadata(agent_id="my-bot", version_label="v1")
...
# =========================================================================
# Identity stamping (p3)
# =========================================================================


class TestIdentityStamping:

    def test_stamps_credential_when_enabled(self, exe_client, mock_session):
        exe_client.stamp_identity = True
        exe_client._buffer.append({"traceId": "tr_1", "agentId": "bot", "status": "success"})
        # First call issues the credential; second is the ingest.
        mock_session.request.side_effect = [
            make_mock_response(
                200,
                {"data": {"credential": "exe_agt_stamp", "expiresAt": "2099-01-01T00:00:00Z"}},
            ),
            make_mock_response(200, {}),
        ]
        ...
        credential_calls = [
            c for c in mock_session.request.call_args_list if "/credential" in str(c)
        ]
        assert len(credential_calls) == 1
        ingest_calls = [
            c for c in mock_session.request.call_args_list if "/api/v1/traces/ingest" in str(c)
        ]
        assert len(ingest_calls) == 1
        traces = ingest_calls[0][1]["json"]["traces"]
        assert traces[0]["agentCredential"] == "exe_agt_stamp"

    def test_no_credential_by_default(self, exe_client, mock_session):
        exe_client._buffer.append({"traceId": "tr_1", "agentId": "bot", "status": "success"})
        mock_session.request.return_value = make_mock_response(200, {})
        ...
        traces = ingest_calls[0][1]["json"]["traces"]
        assert "agentCredential" not in traces[0]

    def test_ingests_even_when_credential_issue_fails(self, exe_client, mock_session):
        exe_client.stamp_identity = True
        exe_client._buffer.append({"traceId": "tr_1", "agentId": "bot", "status": "success"})
        # Credential issue fails (e.g. signing keys not configured); ingest must still happen.
        # The credential _request raises on its single 500; the ingest then succeeds.
        mock_session.request.side_effect = [
            make_mock_response(500, text="signing not configured"),
            make_mock_response(200, {}),
        ]
...
# =========================================================================
# Flush: permanent vs transient ingest failures
# =========================================================================


class TestFlushRetryClassification:
    """A 4xx other than 408/429 rejects the payload for good; retrying only
    delays the drop. Transient failures keep their retries."""

    def _ingest_calls(self, mock_session):
        return [
            c for c in mock_session.request.call_args_list if "/api/v1/traces/ingest" in str(c)
        ]

    def test_permanent_400_is_not_retried(self, exe_client, mock_session, monkeypatch, caplog):
        sleeps = []
        monkeypatch.setattr("execlave.client.time.sleep", lambda s: sleeps.append(s))
        mock_session.request.return_value = make_mock_response(
            400, {"error": {"message": "Validation failed"}}
        )
        exe_client._buffer.append({"traceId": "t1", "agentId": "a", "status": "success"})

        with caplog.at_level("ERROR", logger="Execlave"):
            exe_client._do_flush()

        assert len(self._ingest_calls(mock_session)) == 1
        assert sleeps == []
        assert "dropping 1 traces without retry" in caplog.text
        assert "Validation failed" in caplog.text

    @pytest.mark.parametrize("status", [429, 503])
    def test_transient_status_is_retried(self, exe_client, mock_session, monkeypatch, status):
        monkeypatch.setattr("execlave.client.time.sleep", lambda s: None)
        mock_session.request.side_effect = [
            make_mock_response(status, {"error": {"message": "busy"}}),
            make_mock_response(202, {}),
        ]
        exe_client._buffer.append({"traceId": "t1", "agentId": "a", "status": "success"})

        exe_client._do_flush()

        assert len(self._ingest_calls(mock_session)) == 2
