...
Error traces no longer take their batch down with them (parity with
`@execlave/sdk` 1.8.1).

### Fixed

- **`trace.finish("error", message)` without an error type no longer loses
  traces.** The platform rejected an error trace with no `errorType`, which
  failed the whole ingest batch (successful traces included), and the SDK then
  retried and dropped it. Error traces now always carry a type and message: a
  missing or blank value becomes `UnspecifiedError` / `No error message
provided`. The `with exe.trace(...)` context manager already recorded the
  exception class and is unchanged. The platform applies the same placeholders
  server-side, so older SDK versions are covered too once it is updated.
- **Permanent ingest rejections are not retried.** A 4xx response other than
  408/429 (including 401/403) now drops that chunk immediately with a single
  error log naming the status and the server's reason, instead of retrying a
  payload that cannot succeed. 408, 429, 5xx and network errors are retried as
  before. `ExeclaveError` raised for an HTTP failure now carries a
  `status_code` attribute.

## [1.8.0] - 2026-09-21

Enforcement bypasses now reach the platform's audit trail (parity with
`@execlave/sdk` 1.8.0), and the plan-limit bypass is reported like the others.

### Added

- **Enforcement bypasses are now reported to the platform.** Until now
  `on_enforcement_bypassed` told only the caller's own process, so the audit
  trail could not tell "every call was governed" from "the SDK lost contact for
  two hours". The SDK now sends each bypass to `POST /api/v1/sdk/bypass-windows`,
  ...
  - Bypasses are coalesced into windows per (agent, reason) and only closed
    ...
    after 60 s without a bypass, or after 15 min. A long outage is many records,
    never one mutable one.
  - Reporting is off the enforcement path: a background thread (started on the
    first bypass) retries with backoff (10 s, doubling to 5 min) and never delays
    or raises into `enforce_policy`. `shutdown()` makes one bounded (3 s) attempt.
  ...
  - Text is made storable first: a lone surrogate or NUL is replaced with
    U+FFFD, fields are cut to the server's limits counted in UTF-16 units (500
    emoji are 1000 units), and an empty `agent_id` is sent as `unknown`.
  - **On by default.** Pass `report_bypasses_to_platform=False` to keep only the
    local `on_enforcement_bypassed` callback, which is unchanged and fires once
    per bypass regardless.
...
- **`on_enforcement_bypassed` now fires for a fail-open plan-limit allow.**
  When the plan quota is exhausted and `plan_limit_behavior="fail_open"` (the
  default), `enforce_policy` returned `allowed: True` with no event — so a
  caller who had wired the callback got silence on the bypass most likely to
  occur in normal operation. It now emits `reason="plan_limit_exceeded"`,
  `source="fail_open_plan_limit"`, `status=402`, matching the JS SDK. The
  returned dict also carries `"source": "fail_open_plan_limit"`.

### Changed

- **Circuit-breaker bypass `source` is now `fail_open_circuit_breaker`** (was
  `circuit_breaker_fail_open`), in both the bypass event and the returned
  result, to match the JS SDK and the other `fail_open_*` sources. Code that
  compared `result["source"]` or `event["source"]` against the old string must
  be updated.

## [1.7.0] - 2026-08-03

Production audit remediation: enforcement bypass visibility, strict schema
validation, and approval certificate verification evidence.

### Added

- **`on_enforcement_bypassed` config parameter** — register a callback that
  fires on every ungoverned execution (network error, server error, circuit
  breaker open, fail-open allow). Each event dict carries `reason`, `agentId`,
  `timestamp`, and optional `status`/`message`, so the ungoverned window is
  observable even when the governance plane is down.
- **Approval certificate verification evidence** in compliance reports —
  `verified_at_execution` and `verified_at` fields on certificate inventory
  items, plus summary counts (verified vs unverified).
...
- **Server-Timing header** on the enforce endpoint — `enforce;dur=<ms>` for
  client-side latency attribution.

### Fixed

- **Strict schema validation on all 78 request schemas** — unknown fields now
  produce a `400` rather than being silently dropped.
- **Semantic classifier fast-path** was unreachable for `action_type: 'general'`
  ...
  ~300ms.
- **XSS sanitizer no longer corrupts CEL expressions** — markup patterns now
  only apply when `<` is present in the value.
- **`custom` policy type rejected** — was creatable but inert; now returns a
  validation error.
- **Injection detector regex semantics** — added bounded regex execution to
  prevent ReDoS on ambiguous patterns.
...
- **`enforce_policy()` now rejects unknown fields.** The server enforcement
  ...
  list) returns a `400` instead of being silently accepted. If you relied on an
  unrecognized field being ignored, switch to the documented parameter names.

### Added

- **`enforce_tool_output(agent_id, tool_name, output, ...)`** — synchronously
  scan a tool's output _before_ feeding it back to the model. Raises
  `PolicyBlockedError` when a block-mode `tool_output_scan` policy fires, so a
  poisoned or PII-laden tool result (indirect prompt injection) can be stopped
  before the agent consumes it.
- **`conversation_history`** parameter on `enforce_policy()` — pass recent turns
  so injection scanning runs over the combined conversation, catching an attack
  split across multiple turns (a "crescendo") that no single turn reveals.
- **`tool_outputs`** parameter on `enforce_policy()`.

### Fixed

- **`enforcement_on_outage="fail_closed"` now blocks on the first failure.** It
...
- **Server errors (5xx) now honor `enforcement_on_outage`.** A `502`/`503` — the
  common shape of an outage behind a proxy — previously bypassed the setting and
  raised a generic error. It now routes through the same fail-open/fail-closed
  path as a network failure. Fail-open allows carry a `source` marker
  (`fail_open_network_error` / `fail_open_server_error`).
- **The kill switch is no longer bypassed by the local policy cache.** A paused
  ...
  `enforce_policy()` now skips the cache while the agent is paused and the cache
  is flushed the moment a pause is observed.
- **The LangChain and OpenAI Agents integrations now enforce on their primary
  code path.** The LangChain callback handler previously enforced only on chain
  start, so a direct `llm.invoke()` was traced but never gated; it now enforces
  at the LLM boundary (de-duplicated against an already-enforced parent run). The
  OpenAI Agents tracing processor now enforces the agent's own input, not only
  tool calls.
