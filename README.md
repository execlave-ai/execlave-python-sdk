...
> **Framework integrations** — drop in one callback/processor/helper for [LangChain, OpenAI Agents SDK, CrewAI, LlamaIndex, AutoGen, OpenAI Chat Completions, or the Model Context Protocol](https://www.execlave.com/docs/integrations). See the [full docs](https://www.execlave.com/docs/integrations) or [get an API key](https://www.execlave.com/sign-up?utm_source=github&utm_medium=sdk&utm_campaign=python).
...
`autonomy_level` (optional) maps the agent onto a tiered-governance template that
...
> **This option does not block LLM calls — it is a tagging/telemetry feature.** To actually prevent execution when injection is detected, configure an `injection_scan` policy with `enforcement_mode = block` in the dashboard and call `exe.enforce_policy(...)` before your LLM call. See [Policy Enforcement](#policy-enforcement).
...
| `on_enforcement_bypassed`     | `Callable[[dict], None]` | `None`                     | Called once for every call that ran without a server enforcement decision (network error, 5xx, circuit breaker open, or plan limit under fail-open).                                                                          |
| `report_bypasses_to_platform` | `bool`                   | `True`                     | Also record each ungoverned window (coalesced per agent and reason) in your organisation's audit log as `enforcement.bypassed`. Set `False` to keep only the local callback. Requires 1.8.0+ and a platform that supports it. |
