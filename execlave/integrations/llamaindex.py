    ...
    def _open(self, event: Any, cls_name: str, kind: str, span_id: str) -> None:
        ...
        if cls_name == "AgentToolCallEvent" and self._enforce:
            ...
            try:
                ...
                # action context, so the untruncated tool input goes in
                # metadata too.
                enforce_policy_bound(
                    self._exe,
                    self._agent_id,
                    truncate_for_classifier(tool_input) or f"tool:{tool_name}",
                    tools=[str(tool_name)],
                    metadata=seal_metadata_entry("toolArguments", tool_input),
                )
            except EnforcementHaltError:
                raise
            except Exception:
            ...
            try:
                # query may be a structured QueryBundle, not just a plain
                # string -- truncate_for_classifier truncates/flattens it;
                # seal the full value into metadata too.
                enforce_policy_bound(
                    self._exe,
                    self._agent_id,
                    truncate_for_classifier(query) or "query",
                    metadata=seal_metadata_entry("query", query),
                )
            except EnforcementHaltError:
                raise
            except Exception:
