...
def instrument_autogen_agent(
    ...
    *,
    ...
    def _enforce_message(messages: Any) -> None:
        if not enforce:
            return
        # _last_user_text only looks at the LAST user/human message (or the
        # last message overall) -- the certificate's digest must cover the
        # WHOLE sealed action context, so the full conversation goes in
        # metadata too: a drift anywhere in prior turns must still
        ...
        try:
            enforce_policy_bound(
                ...
                agent_id,
                text,
                metadata=seal_metadata_entry("messages", messages),
            )
    ...
    def _enforce_tool_calls(reply: Any) -> None:
        ...
        for name, args in _extract_tool_calls(reply):
            try:
                # `input` (truncate_for_classifier) truncates at 4000 chars.
                # The certificate's digest must cover the WHOLE sealed
                # action context, so the untruncated args go in metadata too.
                enforce_policy_bound(
                    exe,
                    agent_id,
                    truncate_for_classifier(args) or f"tool:{name}",
                    tools=[name],
...
def _last_user_text(messages: Any) -> str | None:
    ...
    if messages:
        return truncate_for_classifier(_get(messages[-1], "content"))
    return None
