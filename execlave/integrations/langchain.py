...
from ..errors import EnforcementHaltError, PolicyBlockedError
...
from ._action_binding import seal_metadata_entry, enforce_policy_bound, SealedMetadata
...
class ExeclaveCallbackHandler(_BaseCallbackHandler):  # type: ignore[misc,valid-type]
    ...
    def __init__(
        ...
        *,
        ...
        # Run IDs whose input was already enforced (a top-level chain) or that
        # descend from such a run, so a nested LLM/chat callback is not gated a
        # second time while a direct model.invoke() still gets enforced.
        self._enforced_runs: set[UUID] = set()
    ...
    def _close(
        ...
        *,
        ...
        self._enforced_runs.discard(run_id)
    ...
    def _is_covered(self, parent_run_id: UUID | None) -> bool:
        """A run is covered when an ancestor chain already enforced its input."""
        return parent_run_id is not None and parent_run_id in self._enforced_runs
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        if input_text and parent_run_id is None:
            # _extract_input_text only pulls known keys or joins top-level
            # string values -- nested objects and non-string fields are
            # silently dropped from what gets bound. The certificate's
            # digest must cover the WHOLE sealed action context, so the
            # full raw inputs dict is sealed into metadata via the shared
            # _action_binding helper too.
            # Only enforce on top-level chain entry to avoid N x enforcement.
            self._enforce_call(
                input_text,
                metadata=seal_metadata_entry("chainInputs", inputs),
            )
        if parent_run_id is None:
            # Mark this run enforced so nested LLM/chat callbacks skip re-gating.
            self._enforced_runs.add(run_id)
        elif parent_run_id in self._enforced_runs:
            # Propagate coverage down: a child chain under an enforced ancestor
            # is itself covered.
            self._enforced_runs.add(run_id)
    ...
    def on_llm_start(
        ...
        **kwargs: Any,
        ...
        # -- i.e. a direct llm.invoke() with no wrapping chain. Without this the
        # most common call shape is traced but never gated, so a block-mode
        # policy silently fails to stop the LLM call.
        if not self._is_covered(parent_run_id):
            input_text = "\n".join(p for p in prompts if isinstance(p, str) and p)
            if input_text:
                self._enforce_call(
                    input_text,
                    metadata=seal_metadata_entry("llmPrompts", prompts),
                )
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        # Same LLM-boundary enforcement as on_llm_start, for chat models.
        # _stringify_messages returns a list[dict] for span input; for the
        # enforce call we need the flat text the model will actually receive.
        if not self._is_covered(parent_run_id):
            stringified = _stringify_messages(messages)
            input_text = "\n".join(
                m["content"] for m in stringified if m.get("content")
            )
            if input_text:
                self._enforce_call(
                    input_text,
                    metadata=seal_metadata_entry("chatMessages", messages),
                )
