...
def _record_step(
    ...
    if kind == SPAN_KIND_TOOL and enforce and tool:
        try:
            # `input` (truncate_for_classifier) is a bounded view for
            # policy/classifier read. The certificate's digest must cover
            # the WHOLE sealed action context, so the untruncated tool
            # input goes in metadata too -- this adapter previously called
            # exe.enforce_policy directly with no metadata at all, so a
            # require_approval certificate for a CrewAI tool call bound to
            # nothing but a str()'d, unbounded-but-untruncated input string.
            enforce_policy_bound(
                exe,
                agent_id,
                truncate_for_classifier(tool_input) or f"tool:{tool}",
                tools=[str(tool)],
                metadata=seal_metadata_entry("toolInput", tool_input),
            )
        except EnforcementHaltError:
            raise
        except Exception:
