...
from ._action_binding import truncate_for_classifier, seal_full_request, enforce_policy_bound
    ...
    *,
    ...
    def _extract_user_input(messages: Any) -> str | None:
        if not isinstance(messages, list):
            return truncate_for_classifier(messages)
        for msg in reversed(messages):
            ...
            if role == "user":
                ...
                if isinstance(content, list):
                    ...
                    if parts:
                        return truncate_for_classifier("\n".join(parts))
                return truncate_for_classifier(content)
        return None

    def _before(model: str | None, messages: Any, full_request: dict) -> Any:
        if enforce:
            user_text = _extract_user_input(messages) or "chat.completions"
            try:
                # `input` is the extracted latest user turn — bounded for
                # policy/classifier read, not the whole request. The
                # certificate's digest covers the WHOLE sealed action context,
                # so the ENTIRE request kwargs (model, messages, tools,
                # tool_choice, temperature, response_format, ...) go into
                # metadata — not a hand-picked subset that stops covering a
                # field the day OpenAI adds one and this adapter isn't
                # updated.
                enforce_policy_bound(
                    exe,
                    agent_id,
                    user_text,
                    metadata=seal_full_request(full_request),
                )
    ...
    if is_async:
        ...
        async def wrapped_create_async(*args: Any, **kwargs: Any) -> Any:
            ...
            span = _before(model, messages, kwargs)
            try:
                resp = await original_create(*args, **kwargs)
            except Exception as exc:
                span.finish(status="error", error_message=truncate_for_classifier(exc), error_type=type(exc).__name__)
                raise
            _after(span, resp)
            return resp
