...
class Trace:
    ...
    def set_tokens(
        ...
        *,
        ...
        """Set token counts for input/output.

        Accepts either Python-style ``input``/``output`` or JS-SDK-style
        ``prompt``/``completion`` keyword arguments for cross-language parity.
        """
        self._input_tokens = input if input is not None else (prompt if prompt is not None else 0)
        self._output_tokens = output if output is not None else (completion if completion is not None else 0)
        return self
    ...
    def add_tags(self, tags: list[str]) -> "Trace":
        """Append tags (deduplicated). Chainable."""
        if not tags:
            return self
        for tag in tags:
    ...
    def finish(
        ...
        Args:
            status: 'success' | 'error' | 'timeout' | 'policy_blocked' |
                    'limit_exceeded' | 'flagged_for_review'
        ...
        # An error trace must carry a type and message. finish("error", msg) is
        # a documented call and omits the type; older platforms rejected such a
        # trace and, with it, the whole batch. Use the platform's placeholders
        # rather than lose an observed error.
        if self._status == "error":
            self._error_type = (self._error_type or "").strip() or "UnspecifiedError"
            self._error_message = (self._error_message or "").strip() or "No error message provided"
    ...
    def __call__(self, func):
        """
        Allow a Trace to be used as a decorator. ``@exe.trace(session_id=...)``
        returns a Trace; Python then applies it to the decorated function.
        Each invocation gets its own fresh Trace so concurrent calls don't
        clobber each other.
        """
        import functools

        # Capture trace configuration so each invocation gets a fresh Trace.
        exe = self._exe
        ...
        # Mark the original Trace as finished so it doesn't get auto-flushed
        # by garbage collection — it was a config carrier, not a real trace.
        self._finished = True
        return wrapper
