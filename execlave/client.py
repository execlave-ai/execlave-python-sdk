...
class Execlave:
    ...
    def _request(self, method: str, path: str, **kwargs) -> dict:
        ...
        if resp.status_code in (401, 403):
            auth_error = ExeclaveAuthError("Invalid API key or insufficient permissions")
            auth_error.status_code = resp.status_code  # type: ignore[attr-defined]
            raise auth_error
        ...
        if not resp.ok:
            ...
            try:
                ...
                msg += f": {resp.text[:200]}"
            api_error = ExeclaveError(msg)
            # Lets callers tell a permanent rejection (4xx) from a transient one.
            api_error.status_code = resp.status_code  # type: ignore[attr-defined]
            raise api_error
        ...
        for i in range(0, len(batch), self.batch_size):
            ...
            while retries < 3:
                try:
                    ...
                    # A 4xx other than 408/429 rejects this exact payload for
                    # good: retrying cannot succeed, so drop once, loudly.
                    status = getattr(e, "status_code", None)
                    if status is not None and 400 <= status < 500 and status not in (408, 429):
                        logger.error(
                            "Trace ingestion rejected (%d); dropping %d traces without retry: %s",
                            status,
                            len(chunk),
                            e,
                        )
