...
class Execlave:
    ...
    def _cache_key(self, agent_id: str, input_text: str, environment: str) -> str:
        h = hashlib.sha256(
            f"{environment}:{agent_id}:{input_text}".encode()
        ).hexdigest()[:16]
        return f"policy:{h}"
    ...
    def enforce_policy(
        ...
        *,
        ...
        # 1. Check cache first
        cache_key = self._cache_key(agent_id, input, effective_environment)
    ...
    def _poll_approval_decision(self, approval_request_id: str) -> dict:
        ...
        while time.time() - started_at < timeout_seconds:
            ...
            if status == "approved":
                result = {"allowed": True, "approvalRequestId": approval_request_id}
                if "certificate" in approval:
                    result["certificate"] = approval["certificate"]
                return result
            if status == "denied":
